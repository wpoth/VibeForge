...
function ConfidenceBadge({ confidence }: { confidence: SongResearch["confidence"] }) {
    const label =
        confidence === "high"
            ? "High confidence"
            : confidence === "medium"
        ...
        <AnimatePresence>
            {open && (
                <>
                    <motion.button
                        type="button"
                        aria-label="Close song research"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        onClick={onClose}
                            ...
                            {!loading && !error && research && (
                                <div className="space-y-5">
                                    ...
                                    <p className="text-xs leading-5 text-zinc-600">
                                        Interpretations are generated from public music knowledge
                                        and metadata. VibeForge avoids reproducing full lyrics.
                                    </p>
                                </div>
                            )}
                        </div>
